...
export const renderMarkup = function (searchedEvents) {
  ...
  for (let i = 0; i < totalEl; i++) {
    render += ` <li class="card__item" id="${searchedEvents._embedded.events[i].id}">
            ...
            <h2 class="card__item__name-of-group card__item__elements card__item__animation ${searchedEvents._embedded.events[i].name.length > 20 ? "j-content" : ""}"><span id="name-marquee" ${searchedEvents._embedded.events[i].name.length > 20 ? "class='marquee'" : ""}> 
            ${
            ...
            <a href="https://www.google.com/maps" target="_blank" rel="noopener noreferrer" class="card__item__location card__item__elements card__item__animation center-location ${searchedEvents._embedded.events[i]._embedded.venues.map(item =>
